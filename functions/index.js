...
exports.getEngagementSnapshot = onCall({region: REGION, cors: true}, async (request) => {
  ...
  const [catalogSnap, earnedSnap, progressSnap, statsSnap, todaySnap, freshProfile] = await Promise.all([
  ...
  return {
    ...
    catalog,
    earned,
    stats: statsSnap.exists ? serializeValue(statsSnap.data()) : {},
    checkIn: {today, checkedIn: todaySnap.exists},
    profilePatch: {
...
exports.dailyCheckIn = onCall({region: REGION, cors: true}, async (request) => {
  ...
  if (!result.alreadyCheckedIn && actor.data.role !== "tester") {
...
async function processOfficialEventLog(log, sourceLogId, sourceType = "official_tournament", dryRun = false) {
  ...
  await evaluateTitles(uid, sourceType, eventCode);
  return {status: created ? "created" : "skipped", reason: created ? null : "already-processed", uid, eventCode};
}
...
exports.getEngagementAdminOverview = onCall({region: REGION, cors: true}, async (request) => {
  ...
  return {
    ...
    topStreaks: topStats.docs.map((doc, index) => ({uid: doc.id, name: String((topUsers[index].data() || {}).realName || (topUsers[index].data() || {}).displayName || doc.id), stats: serializeValue(doc.data())})),
...
exports.runTitleBackfill = onCall({region: REGION, cors: true, timeoutSeconds: 120}, async (request) => {
  ...
  const summary = {scanned: snapshot.size, eligible: 0, created: 0, skipped: 0, failed: 0};
  const errors = [];
  ...
  const nextCursor = snapshot.empty ? cursor : snapshot.docs[snapshot.docs.length - 1].id;
  const done = snapshot.size < limit;
  const runId = safeId(`${actor.uid}_${Date.now()}_${execute ? "execute" : "preview"}`);
  ...
  return {ok: true, runId, mode: execute ? "execute" : "preview", nextCursor: done ? null : nextCursor, done, summary, errors: errors.slice(0, 20)};
});
...
exports.seedInitialTitles = onCall({region: REGION, cors: true}, async (request) => {
  ...
  for (const item of definitions) {
    ...
    batch.set(db.doc(`titleDefinitions/${item.id}`), definition, {merge: true});
    batch.set(db.doc(`publicTitleCatalog/${item.id}`), publicDefinition(item.id, definition), {merge: true});
  }
  await batch.commit();
...
exports.saveEngagementSettings = onCall({region: REGION, cors: true}, async (request) => {
  ...
  const payload = {
    ...
    timezone: "Asia/Taipei",
    launchAt: data.launchAt ? Timestamp.fromMillis(Number(data.launchAt)) : null,
    updatedAt: FieldValue.serverTimestamp(),
    updatedBy: actor.uid,
  };
